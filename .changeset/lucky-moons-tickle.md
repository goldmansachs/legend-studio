...
The failed Python panel now shows the shared component's fixed wording instead of the exception message the forked panel rendered verbatim. The specific reason is logged against `AI_AGENT_PYTHON_CODEGEN_FAILURE` rather than shown, matching how the sibling DataCube translation failure is already handled.
